import { pgTable, uuid, integer, numeric, varchar, boolean, timestamp, uniqueIndex } from 'drizzle-orm/pg-core'
import { dailyEggRecords } from './daily-egg-records'

export const dailyEggBundles = pgTable('daily_egg_bundles', {
  id: uuid('id').primaryKey().defaultRandom(),
  dailyEggRecordId: uuid('daily_egg_record_id').notNull().references(() => dailyEggRecords.id, { onDelete: 'cascade' }),
  ...
  recordIndexUnique: uniqueIndex('daily_egg_bundles_record_index_unique').on(t.dailyEggRecordId, t.bundleIndex),
}))
