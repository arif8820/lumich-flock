...
export function EditCustomerForm({ customer, onSuccess, onCancel }: Props) {
  ...
  const [creditLimit, setCreditLimit] = useState(String(Number(customer.creditLimit)))
  const [paymentTerms, setPaymentTerms] = useState(String(customer.paymentTerms))
  ...
  return (
    <form onSubmit={onSubmit} className="p-4 border border-[var(--lf-border)] rounded-xl bg-[var(--lf-bg)] space-y-3">
      <h3 className="text-sm font-semibold text-[var(--lf-text-dark)]">Edit Pelanggan</h3>
