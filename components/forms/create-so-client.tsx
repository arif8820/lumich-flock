...
export function CreateSOClient({ customers, role }: Props) {
  ...
  const [draft, setDraft] = useState<DraftSO>(() => {
    ...
    if (saved) {
      try {
        return JSON.parse(saved) as DraftSO
      } catch {}
    }
    return defaultDraft
  })
  ...
  const [restored] = useState(() => {
    ...
    return !!sessionStorage.getItem('so_draft')
  })
  ...
  const updateItem = (index: number, field: keyof SalesOrderItem, value: unknown) => {
    ...
    newItems[index] = { ...newItems[index]!, [field]: value } as SalesOrderItem
  ...
  const subtotal = draft.items.reduce(
    (sum, item) => sum + item.quantity * item.pricePerUnit * (1 - item.discountPct / 100),
    0
  )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasZeroPrice = draft.items.some((item) => item.pricePerUnit === 0)
    if (hasZeroPrice) {
      const confirmed = window.confirm('Terdapat item dengan harga Rp 0. Lanjutkan?')
      if (!confirmed) return
    }
    ...
    if (draft.overrideReason) formData.append('overrideReason', draft.overrideReason)
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setDraft({ ...draft, paymentMethod: e.target.value as 'cash' | 'credit' })
              }
              className="w-full px-3 py-2 border rounded"
            >
