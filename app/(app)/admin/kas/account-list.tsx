...
const TYPE_LABEL: Record<string, string> = { cash: 'Tunai', bank: 'Bank', ewallet: 'E-Wallet' }

export function AccountList({ accounts }: { accounts: AccountWithTx[] }) {
  if (accounts.length === 0) {
    return (
      <div className="px-4 py-6 text-center">
        <p className="text-[12px]" style={{ color: '#8fa08f' }}>Belum ada akun kas.</p>
      </div>
    )
  }
...
function AccountRow({ account }: { account: AccountWithTx }) {
  ...
  if (!editing) {
    return (
      <div className="flex items-center justify-between px-4 py-2.5">
        <div>
          <p className="text-[13px] font-medium" style={{ color: '#2d3a2e' }}>{account.name}</p>
          <p className="text-[11px]" style={{ color: '#8fa08f' }}>{TYPE_LABEL[account.type] ?? account.type}</p>
        </div>
    ...
    <form action={handleSubmit} className="px-4 py-3 space-y-2">
      ...
      <div className="flex gap-2">
        ...
        <label className="flex items-center gap-1.5 text-[12px]" style={{ color: '#5a6b5b' }}>
          <input type="checkbox" name="isActive" value="true" defaultChecked={account.isActive} />
          Aktif
        </label>
      </div>
      {account.hasTx ? (
        <p className="text-[11px] px-2 py-1.5 rounded-lg" style={{ background: '#f7f5f1', color: '#8fa08f' }}>
          Saldo awal tidak dapat diubah — sudah ada transaksi
        </p>
      ) : (
        <input
          ...
          min="0"
          step="1"
          defaultValue={String(account.beginningBalance)}
